...
function getCategorizedSessions(sessions: SessionData[]): CategorizedSessions {
  ...
  for (const session of sessions) {
    ...
    if (sessionDayStart >= thirtyDaysAgoStart && sessionDayStart < sevenDaysAgoStart) {
...
function renderSessions(props: {
  ...
  onSelectSession: (event: MouseEvent<HTMLAnchorElement>, sessionId: string) => void;
      ...
      <SidebarContent className="no-scrollbar overscroll-contain">
        {props.sessions.length === 0 ? (
          <SidebarGroup className="group-data-[collapsible=icon]:hidden">
            ...
            <div className="px-2 text-xs text-muted-foreground">No local sessions yet.</div>
          </SidebarGroup>
        ) : null}
        {categories.map((category) => {
          ...
          return (
            <SidebarGroup key={category} className="group-data-[collapsible=icon]:hidden">
