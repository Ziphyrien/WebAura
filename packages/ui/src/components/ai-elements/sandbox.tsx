...
export const Sandbox = ({ className, ...props }: SandboxRootProps) => (
  <Collapsible
    className={cn(
      "not-prose group mb-4 w-full overflow-hidden rounded-md border border-border",
      className,
    )}
    defaultOpen
    {...props}
...
export const SandboxTabsBar = ({ className, ...props }: SandboxTabsBarProps) => (
  <div
    className={cn("flex w-full items-center border-t border-b border-border", className)}
    {...props}
  />
);
