...
import { serializeRuntimeError } from "@gitinspect/pi/agent/runtime-error-payload";
import {
...
class WorkerAgentRunner {
  ...
  async refreshGithubToken(token: RefreshGithubTokenInput["token"]): Promise<void> {
    if (this.disposed) {
      return;
    }
  ...
  private async flushSnapshotNow(): Promise<void> {
    ...
    const envelope: WorkerSnapshotEnvelope = {
      ...
      runtimeErrors:
        this.pendingRuntimeErrors.length > 0 ? [...this.pendingRuntimeErrors] : undefined,
...
export async function startTurn(input: StartTurnInput, events: RuntimeWorkerEvents): Promise<void> {
...
export async function waitForTurn(sessionId: string): Promise<WorkerSnapshotEnvelope | undefined> {
...
export async function setModelSelection(input: ConfigureSessionInput): Promise<void> {
  await getRunner(input.sessionId)?.setModelSelection(input.providerGroup, input.modelId);
}
...
export async function refreshGithubToken(input: RefreshGithubTokenInput): Promise<void> {
