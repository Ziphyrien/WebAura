...
export class MissingSessionRuntimeError extends RuntimeCommandError {
  ...
  constructor(sessionId: string) {
    super(MISSING_SESSION_RUNTIME_ERROR_NAME, `Missing runtime session: ${sessionId}`, sessionId);
  }
}
...
export class BusyRuntimeError extends RuntimeCommandError {
  ...
  constructor(sessionId: string) {
    super(BUSY_RUNTIME_ERROR_NAME, `Runtime session is busy: ${sessionId}`, sessionId);
  }
}
...
export function reviveRuntimeCommandError(error: Error, sessionId?: string): Error {
...
export function getRuntimeCommandErrorMessage(error: Error | undefined): string {
  if (!error) {
  ...
  if (lower.includes("too many requests") || lower.startsWith("429") || lower.includes(" 429")) {
