...
export function isSessionLeaseStale(lease: SessionLeaseRow, now = Date.now()): boolean {
  return now - toTimestamp(lease.heartbeatAt) >= LEASE_STALE_MS;
}

export async function loadSessionLeaseState(sessionId: string): Promise<SessionLeaseState> {
...
export async function claimSessionLease(sessionId: string): Promise<LeaseClaimResult> {
  ...
  await db.transaction("rw", db.sessionLeases, async () => {
    ...
    if (current && current.ownerTabId !== ownerTabId && !isSessionLeaseStale(current)) {
      return;
    }
...
export async function renewSessionLease(sessionId: string): Promise<SessionLeaseRow | undefined> {
  const ownerTabId = getCurrentTabId();
...
export async function releaseOwnedSessionLeases(): Promise<void> {
  const ownerTabId = getCurrentTabId();
  const leases = await db.sessionLeases.where("ownerTabId").equals(ownerTabId).toArray();
